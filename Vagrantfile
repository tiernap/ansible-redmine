...
Vagrant::configure(VAGRANTFILE_API_VERSION) do |global_config|
  ...
  global_config.hostmanager.enabled = true
   ...
   global_config.vm.define("redmine.internal.tld") do |config|
    config.hostmanager.enabled = true
    config.hostmanager.manage_host = true
    config.hostmanager.ignore_private_ip = false
    config.hostmanager.include_offline = true
    config.vm.network :private_network, ip: "10.0.1.201"
    config.vm.hostname = "redmine.internal.tld"
    ...
    config.vm.provider :virtualbox do |vb|
      ...
      vb.customize [ "createhd", "--filename", "disk-redmine", "--size", "5000" ]
      vb.customize [ "storageattach", :id, "--storagectl", "SATAController", "--port", 4, "--device", 0, "--type", "hdd", "--medium", "disk-redmine.vdi" ]
    end
